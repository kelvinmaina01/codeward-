...
async function main() {
  console.log('=== ARCHITECTURE AGENT — real LocalExecSandbox, fresh clone, real LLM ===\n');
  ...
  try {
    const toolMap = architectureAgent.createTools(sandbox);
    ...
    const toolArray = Object.entries(toolMap).map(([name, impl]) => ({
      ...
      execute: async (args: any) => {
        ...
        if (name === 'submit_architecture_report') reportResult = args;
        return result;
      }
    }));
    ...
    await runAgentLoop({
      ...
      messages: [{
        role: 'user',
        content: `Analyze this repository's architecture and scalability.
Run ID: test-run-architecture-001
There is NO running instance and NO databaseUrl available in this pipeline (clone-and-analyze only) — tools needing a live baseUrl/databaseUrl will honestly report applicable:false, treat that as "not tested", not "passed".
Submit your findings using submit_architecture_report.`
      }]
    }, provider);
    ...
    if (!reportResult) {
      console.log('Agent never called submit_architecture_report.');
      process.exitCode = 1;
    } else {
